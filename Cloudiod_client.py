    ...
    try:
        ...
        if response.ok:
            output = "Thread : {},  input image: {},  output:{}".format(
                threading.current_thread().name, image, response.text
            )
            print(output)
        else:
...
def main():
    ...
    if len(sys.argv) != 4:
        raise ValueError(
            "Arguments list is wrong. Please use the following format: {} {} {} {}".format(
                "python Cloudiod_client.py",
                "<input_folder>",
                "<URL>",
                "<number_of_workers>",
            )
    ...
    print(
        "Total time spent: {} average response time: {}".format(
